...
variable "machine_tuning" {
  description = "Talos machine-level tuning (kernel, kubelet, shutdown). Defaults follow production best practices."
  type = object({
...
variable "cluster_tuning" {
  ...
  type = object({
    kubeconfig_cert_lifetime  = optional(string, "48h0m0s")
